...
public class TrainMode {
	...
	public TrainMode(String folderPath, int samples) {
		path = folderPath;
		sampleSize = samples;
		getTrainPictures();
	}
	...
	private void getTrainPictures() {
		...
		String clusterPath = path + "\\clustered_pictures\\";
		...
		if (folder.isDirectory()) {
			...
			while (!isGenerated()){
				...
				String tempClusterPath = clusterPath + listOfFiles[fileNumber].getName().replaceAll(".jpg|.JPG|.jpeg|.JPEG", ".bmp");
				File gtFile = new File(tempGtPath);
				File stdevFile = new File(tempStdevPath);
				File clusterFile = new File(tempClusterPath);
				if (gtFile.exists() && stdevFile.exists() && clusterFile.exists()) {
					generateRandomTrainSample(listOfFiles[fileNumber].getAbsolutePath(),gtFile.getAbsolutePath(), stdevFile.getAbsolutePath(), clusterFile.getAbsolutePath());
	...
	private void generateRandomTrainSample(String picPath, String gtPath, String stdevPath, String clusterPath) {
		Mat pic = Highgui.imread(picPath);
		Mat gtPic = Highgui.imread(gtPath);
		Mat stdevPic = Highgui.imread(stdevPath);
		Mat clusterPic = Highgui.imread(clusterPath);
		Mat hsvPic = new Mat();
		Mat labPic = new Mat();
		...
		while (sample < 200) {
			...
			if ((classUpperLeft != -1) && (isSameClass == 0) && classes[classUpperLeft] <= sampleSize) {
				...
				networkInput.push_back(getTrainSampleInput(pic, hsvPic, labPic, stdevPic, clusterPic, x, y));
				networkOutput.push_back(getTrainSampleOutput(classUpperLeft));
	...
	protected Mat getTrainSampleInput(Mat pic, Mat hsvPic, Mat labPic, Mat stdevPic, Mat clusterPic, int x, int y) {
			...
			for (int j = 0; j < 7; j++) {
				if (((i%2 == 0) && (j%2 == 0)) || ((i%2 == 1) && (j%2 == 1))) {
					...
					case 0:
						...
						break;
					case 1:
						inputElement.put(0, n, clusterPic.get(x+i, y+j));
						picChoice = 2;
						break;
					case 2:
						inputElement.put(0, n, labPic.get(x+i,y+j));
	...
	protected Mat getTrainSampleOutput(int classN) {
	...
	public void makeRPropNeuralNetwork() {
		...
		TermCriteria criteria = new TermCriteria(TermCriteria.COUNT + TermCriteria.EPS, 5000, 0.0000001);
	...
	protected double getIntensityValue(int classN) {
